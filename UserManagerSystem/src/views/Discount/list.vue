	...
	<div class="list">
		...
		<el-card class="box-card">
			<el-table :data="tableData" width="100%" :default-sort="{prop:'name'}">
				...
				<el-table-column header-align="left" sortable width="70px" prop="id" label="id">
				</el-table-column>
				<el-table-column width="140px" sortable header-align="left"
				 prop="name" label="name"> </el-table-column>
				<el-table-column header-align="left" prop="online" label="online"
				 width="100px"> </el-table-column>
				<el-table-column header-align="left" sortable prop="starttime"
				 width="160px" label="starttime">
					<template slot-scope="scope"> {{scope.row.starttime}}时 </template>
				</el-table-column>
				<el-table-column header-align="left" sortable prop="endtime" width="160px" label="endtime">
					<template slot-scope="scope"> {{scope.row.endtime}}时 </template>
				</el-table-column>
				<el-table-column label="list" width="500px" cell-class-name="center" header-align="center">
					<template slot-scope="scope">
						<el-table :data="scope.row.list">
							<el-table-column cell-style="text-align:center" header-align="center" label="level"
							 width="80px">
								<template slot-scope="scope1"> {{scope1.row.level}} </template>
							</el-table-column>
							<el-table-column cell-style="text-align:center" header-align="center" label="qty"
							 width="200px">
								<template slot-scope="scope1"> {{scope1.row.qty}} </template>
							</el-table-column>
							<el-table-column cell-style="text-align:center" header-align="center" label="discount"
							 width="200px">
								<template slot-scope="scope1"> {{scope1.row.discount}} </template>
							</el-table-column>
							<el-table-column cell-style="text-align:center" header-align="center" label="selltext"
							 width="200px">
								<template slot-scope="scope1"> {{scope1.row.selltext}} </template>
							</el-table-column>
						</el-table>
					</template>
				</el-table-column>
...
<script>
	export default {
		...
		methods: {
			...
			//删除
			handleDelete(item) {
				this.$delete("/admin/v1/content?type=Discount&id=" + item.id, {}).then(
					response => {
						if (response.retCode == 0) {
							this.$message({
								message: '删除成功!',
			...
			queryTable() {
				this.$get('/admin/v1/contents?type=Discount&offset=' + this.pageNum +
